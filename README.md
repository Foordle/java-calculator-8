...
| **InputValidator** | 입력값 검증 | - 빈 문자열(null, "") 처리<br>- 문자열 배열을 순회하며 숫자 형식(Integer.parseInt) 검증<br>- 0 이하(0 및 음수) 입력 시 IllegalArgumentException 발생<br>- 숫자가 아닌 값 입력 시<br>NumberFormatException ->  IllegalArgumentException처리 | 숫자 유효성 검사 기능을 NumberSummer에서 분리하여 책임 분리(Validation 전담) |     
...
| 클래스명               | 주요 변경 내용           | 세부 구현                                                                                                                                  | 비고          |                                                                    
| ------------------ | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| **Calculator**     | 기존 로직 재사용                | 입력이 `//`로 시작하는 경우에도 동일한 흐름 수행<br>(검증 → 분리 → 합산)                                                                                            | 구조 변경 없음    |                                                                    
| **StringSplitter** | 커스텀 구분자 파싱 기능 추가     | - `splitWithCustomDelimiter()` 메서드 추가<br>- `//`로 시작하는 입력에서 `\n` 이전 문자열을 구분자로 인식<br>- `Pattern.quote()`를 사용하여 정규식 특수문자(`?`, `*`, `+`, ` \| `)도 안전하게 처리 | `Pattern.quote()`는 내부적으로 `\Q`와 `\E` 사이의 문자열을 **리터럴(그대로의 문자)** 로 취급 |                                
| **NumberSummer**   | 기존 합산 로직 재사용            | 커스텀 구분자 입력 시에도 동일한 합산 처리 수행                                                                                                            | 수정 없음       |                                                                    
